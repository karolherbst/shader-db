...
uniform vec3 unf59;
const float cons63 = float(20.000000);
uniform vec3 unf66;
const float cons69 = float(0.996195);
const float cons70 = float(0.000571);
const float cons81 = float(1.000000);
uniform sampler2DShadow samp1;
uniform mat4 unf85;
const float cons86 = float(0.005000);
uniform float unf91;
uniform vec3 unf98;
const float cons102 = float(20.000000);
uniform vec3 unf105;
const float cons108 = float(0.996195);
const float cons109 = float(0.000571);
const float cons120 = float(1.000000);
uniform sampler2DShadow samp2;
uniform mat4 unf124;
const float cons125 = float(0.005000);
uniform float unf130;
const float cons137 = float(1.000000);
uniform vec4 unfobcolor;
const float cons143 = float(6.000000);
const float cons144 = float(26.000000);
const float cons145 = float(0.000000);
const float cons146 = float(0.000000);
const vec4 cons150 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	set_rgba(tmp8, tmp11);
	mtex_alpha_from_col(tmp8, tmp13);
	mtex_rgb_blend(cons14, tmp11.rgb, tmp13, cons17, tmp18);
	set_rgb_zero(tmp19);
	lamp_visibility_sun_hemi(unf20, tmp21, tmp22, tmp23);
	shade_inp(tmp2, tmp21, tmp26);
	shade_is_hemi(tmp26, tmp28);
	shade_visifac(tmp28, tmp23, cons31, tmp32);
	shade_mul_value(tmp32, unf34, tmp35);
	shade_madd(vec4(tmp19, 1.0), tmp35, vec4(tmp18, 1.0), tmp39);
	lamp_visibility_sun_hemi(unf40, tmp41, tmp42, tmp43);
	shade_inp(tmp2, tmp41, tmp46);
	shade_visifac(tmp46, tmp43, cons49, tmp50);
	shade_mul_value(tmp50, unf52, tmp53);
	shade_madd(tmp39, tmp53, vec4(tmp18, 1.0), tmp57);
	lamp_visibility_other(varposition, unf59, tmp60, tmp61, tmp62);
	lamp_falloff_invlinear(cons63, tmp61, tmp65);
	lamp_visibility_spot_circle(unf66, tmp60, tmp68);
	lamp_visibility_spot(cons69, cons70, tmp68, tmp65, tmp73);
	lamp_visibility_clamp(tmp73, tmp75);
	shade_inp(tmp2, tmp60, tmp78);
	shade_visifac(tmp78, tmp75, cons81, tmp82);
	test_shadowbuf(varposition, samp1, unf85, cons86, tmp78, tmp88);
	shade_only_shadow(tmp82, tmp88, unf91, tmp92);
	shade_only_shadow_diffuse(tmp92, tmp18, tmp57, tmp96);
	lamp_visibility_other(varposition, unf98, tmp99, tmp100, tmp101);
	lamp_falloff_invlinear(cons102, tmp100, tmp104);
	lamp_visibility_spot_circle(unf105, tmp99, tmp107);
	lamp_visibility_spot(cons108, cons109, tmp107, tmp104, tmp112);
	lamp_visibility_clamp(tmp112, tmp114);
	shade_inp(tmp2, tmp99, tmp117);
	shade_visifac(tmp117, tmp114, cons120, tmp121);
	test_shadowbuf(varposition, samp2, unf124, cons125, tmp117, tmp127);
	shade_only_shadow(tmp121, tmp127, unf130, tmp131);
	shade_only_shadow_diffuse(tmp131, tmp18, tmp96, tmp135);
	mtex_alpha_to_col(tmp135, cons137, tmp138);
	shade_obcolor(tmp138, unfobcolor, tmp141);
	shade_mist_factor(varposition, cons143, cons144, cons145, cons146, tmp147);
	mix_blend(tmp147, tmp141, cons150, tmp151);
	shade_alpha_opaque(tmp151, tmp153);
	shade_alpha_obcolor(tmp153, unfobcolor, tmp156);
