...
uniform vec3 unf23;
const float cons34 = float(0.650000);
uniform vec4 unf37;
uniform sampler1D samp1;
const float cons50 = float(1.000000);
uniform vec3 unf60;
const float cons69 = float(0.650000);
uniform vec4 unf72;
uniform sampler1D samp2;
const float cons85 = float(1.000000);
uniform vec3 unf96;
const float cons100 = float(20.000000);
uniform vec3 unf103;
const float cons106 = float(0.996195);
const float cons107 = float(0.000571);
const float cons118 = float(0.650000);
uniform sampler2DShadow samp3;
uniform mat4 unf122;
const float cons123 = float(0.005000);
uniform float unf128;
uniform vec3 unf135;
const float cons139 = float(20.000000);
uniform vec3 unf142;
const float cons145 = float(0.996195);
const float cons146 = float(0.000571);
const float cons157 = float(0.650000);
uniform sampler2DShadow samp4;
uniform mat4 unf161;
const float cons162 = float(0.005000);
uniform float unf167;
const float cons174 = float(1.000000);
const float cons177 = float(6.000000);
const float cons178 = float(26.000000);
const float cons179 = float(0.000000);
const float cons180 = float(0.000000);
const vec4 cons184 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	shade_is_hemi(tmp29, tmp31);
	shade_visifac(tmp31, tmp26, cons34, tmp35);
	shade_mul_value(tmp35, unf37, tmp38);
	vec_math_dot(tmp4, tmp2, tmp41, tmp42);
	valtorgb(tmp42, samp1, tmp45, tmp46);
	mtex_alpha_from_col(tmp45, tmp48);
	math_multiply(tmp48, cons50, tmp51);
	mix_overlay(tmp51, vec4(tmp21, 1.0), tmp45, tmp55);
	shade_madd(vec4(tmp22, 1.0), tmp38, tmp55, tmp59);
	lamp_visibility_sun_hemi(unf60, tmp61, tmp62, tmp63);
	shade_inp(tmp2, tmp61, tmp66);
	shade_visifac(tmp66, tmp63, cons69, tmp70);
	shade_mul_value(tmp70, unf72, tmp73);
	vec_math_dot(tmp4, tmp2, tmp76, tmp77);
	valtorgb(tmp77, samp2, tmp80, tmp81);
	mtex_alpha_from_col(tmp80, tmp83);
	math_multiply(tmp83, cons85, tmp86);
	mix_overlay(tmp86, vec4(tmp21, 1.0), tmp80, tmp90);
	shade_madd(tmp59, tmp73, tmp90, tmp94);
	lamp_visibility_other(varposition, unf96, tmp97, tmp98, tmp99);
	lamp_falloff_invlinear(cons100, tmp98, tmp102);
	lamp_visibility_spot_circle(unf103, tmp97, tmp105);
	lamp_visibility_spot(cons106, cons107, tmp105, tmp102, tmp110);
	lamp_visibility_clamp(tmp110, tmp112);
	shade_inp(tmp2, tmp97, tmp115);
	shade_visifac(tmp115, tmp112, cons118, tmp119);
	test_shadowbuf(varposition, samp3, unf122, cons123, tmp115, tmp125);
	shade_only_shadow(tmp119, tmp125, unf128, tmp129);
	shade_only_shadow_diffuse(tmp129, tmp21, tmp94, tmp133);
	lamp_visibility_other(varposition, unf135, tmp136, tmp137, tmp138);
	lamp_falloff_invlinear(cons139, tmp137, tmp141);
	lamp_visibility_spot_circle(unf142, tmp136, tmp144);
	lamp_visibility_spot(cons145, cons146, tmp144, tmp141, tmp149);
	lamp_visibility_clamp(tmp149, tmp151);
	shade_inp(tmp2, tmp136, tmp154);
	shade_visifac(tmp154, tmp151, cons157, tmp158);
	test_shadowbuf(varposition, samp4, unf161, cons162, tmp154, tmp164);
	shade_only_shadow(tmp158, tmp164, unf167, tmp168);
	shade_only_shadow_diffuse(tmp168, tmp21, tmp133, tmp172);
	mtex_alpha_to_col(tmp172, cons174, tmp175);
	shade_mist_factor(varposition, cons177, cons178, cons179, cons180, tmp181);
	mix_blend(tmp181, tmp175, cons184, tmp185);
	shade_alpha_opaque(tmp185, tmp187);
