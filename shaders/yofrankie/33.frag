...
const vec3 cons14 = vec3(1.000000, 1.000000, 1.000000);
...
uniform vec3 unf30;
const float cons41 = float(0.800000);
uniform vec4 unf44;
uniform sampler1D samp1;
const float cons57 = float(0.800000);
uniform vec3 unf67;
const float cons76 = float(0.800000);
uniform vec4 unf79;
uniform sampler1D samp2;
const float cons92 = float(0.800000);
const float cons106 = float(6.000000);
const float cons107 = float(26.000000);
const float cons108 = float(0.000000);
const float cons109 = float(0.000000);
const vec4 cons113 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	mtex_rgb_dark(cons14, tmp13.rgb, cons16, cons17, tmp18);
	mtex_alpha_from_col(tmp10, tmp20);
	mtex_value_dark(cons21, cons22, tmp20, cons24, cons25, tmp26);
	...
	vec_math_dot(tmp4, tmp2, tmp48, tmp49);
	valtorgb(tmp49, samp1, tmp52, tmp53);
	mtex_alpha_from_col(tmp52, tmp55);
	math_multiply(tmp55, cons57, tmp58);
	mix_overlay(tmp58, vec4(tmp18, 1.0), tmp52, tmp62);
	shade_madd(vec4(tmp29, 1.0), tmp45, tmp62, tmp66);
	...
	vec_math_dot(tmp4, tmp2, tmp83, tmp84);
	valtorgb(tmp84, samp2, tmp87, tmp88);
	mtex_alpha_from_col(tmp87, tmp90);
	math_multiply(tmp90, cons92, tmp93);
	mix_overlay(tmp93, vec4(tmp18, 1.0), tmp87, tmp97);
	shade_madd(tmp66, tmp80, tmp97, tmp101);
	mtex_alpha_to_col(tmp101, tmp28, tmp104);
	shade_mist_factor(varposition, cons106, cons107, cons108, cons109, tmp110);
	mix_blend(tmp110, tmp104, cons113, tmp114);
