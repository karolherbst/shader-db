...
varying vec2 var0;
const vec3 cons8 = vec3(1.400000, 1.400000, 1.000000);
const vec3 cons11 = vec3(-0.200000, -0.200000, 0.000000);
uniform sampler2D samp0;
const vec3 cons21 = vec3(0.800000, 0.800000, 0.800000);
const float cons24 = float(1.000000);
const vec3 cons29 = vec3(0.500000, 0.600000, 1.000000);
const vec3 cons32 = vec3(0.250000, 0.200000, 0.000000);
uniform sampler2D samp1;
varying vec3 var1;
uniform vec3 unf45;
const float cons56 = float(1.000000);
uniform vec4 unf59;
uniform sampler1D samp2;
const float cons72 = float(1.000000);
const float cons85 = float(0.300000);
const float cons86 = float(109.000000);
uniform vec3 unf90;
const vec3 cons91 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 unf96;
const float cons105 = float(1.000000);
uniform vec4 unf108;
uniform sampler1D samp3;
const float cons121 = float(1.000000);
const float cons134 = float(109.000000);
const float cons136 = float(1.000000);
const float cons137 = float(0.300000);
uniform vec3 unf142;
const vec3 cons143 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 unf149;
const float cons153 = float(20.000000);
uniform vec3 unf156;
const float cons159 = float(0.996195);
const float cons160 = float(0.000571);
const float cons171 = float(1.000000);
uniform sampler2DShadow samp4;
uniform mat4 unf175;
const float cons176 = float(0.005000);
uniform float unf181;
uniform vec3 unf188;
const float cons192 = float(20.000000);
uniform vec3 unf195;
const float cons198 = float(0.996195);
const float cons199 = float(0.000571);
const float cons210 = float(1.000000);
uniform sampler2DShadow samp5;
uniform mat4 unf214;
const float cons215 = float(0.005000);
uniform float unf220;
const float cons230 = float(1.000000);
const float cons233 = float(6.000000);
const float cons234 = float(26.000000);
const float cons235 = float(0.000000);
const float cons236 = float(0.000000);
const vec4 cons240 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	texco_uv(var0, tmp27);
	mtex_mapping_size(tmp27, cons29, tmp30);
	mtex_mapping_ofs(tmp30, cons32, tmp33);
	mtex_image(tmp33, samp1, tmp36, tmp37, tmp38);
	mtex_nspace_tangent(var1, tmp2, tmp38, tmp42);
	set_rgb_zero(tmp43);
	set_rgb_zero(tmp44);
	lamp_visibility_sun_hemi(unf45, tmp46, tmp47, tmp48);
	shade_inp(tmp42, tmp46, tmp51);
	shade_is_hemi(tmp51, tmp53);
	shade_visifac(tmp53, tmp48, cons56, tmp57);
	shade_mul_value(tmp57, unf59, tmp60);
	vec_math_dot(tmp4, tmp42, tmp63, tmp64);
	valtorgb(tmp64, samp2, tmp67, tmp68);
	mtex_alpha_from_col(tmp67, tmp70);
	math_multiply(tmp70, cons72, tmp73);
	mix_overlay(tmp73, vec4(tmp25, 1.0), tmp67, tmp77);
	shade_madd(vec4(tmp43, 1.0), tmp60, tmp77, tmp81);
	shade_hemi_spec(tmp42, tmp46, tmp4, cons85, cons86, tmp48, tmp88);
	shade_add_spec(tmp88, unf90, cons91, tmp92);
	shade_add(vec4(tmp44, 1.0), vec4(tmp92, 1.0), tmp95);
	lamp_visibility_sun_hemi(unf96, tmp97, tmp98, tmp99);
	shade_inp(tmp42, tmp97, tmp102);
	shade_visifac(tmp102, tmp99, cons105, tmp106);
	shade_mul_value(tmp106, unf108, tmp109);
	vec_math_dot(tmp4, tmp42, tmp112, tmp113);
	valtorgb(tmp113, samp3, tmp116, tmp117);
	mtex_alpha_from_col(tmp116, tmp119);
	math_multiply(tmp119, cons121, tmp122);
	mix_overlay(tmp122, vec4(tmp25, 1.0), tmp116, tmp126);
	shade_madd(tmp81, tmp109, tmp126, tmp130);
	shade_phong_spec(tmp42, tmp97, tmp4, cons134, tmp135);
	shade_spec_t(cons136, cons137, tmp99, tmp135, tmp140);
	shade_add_spec(tmp140, unf142, cons143, tmp144);
	shade_add(tmp95, vec4(tmp144, 1.0), tmp147);
	lamp_visibility_other(varposition, unf149, tmp150, tmp151, tmp152);
	lamp_falloff_invlinear(cons153, tmp151, tmp155);
	lamp_visibility_spot_circle(unf156, tmp150, tmp158);
	lamp_visibility_spot(cons159, cons160, tmp158, tmp155, tmp163);
	lamp_visibility_clamp(tmp163, tmp165);
	shade_inp(tmp42, tmp150, tmp168);
	shade_visifac(tmp168, tmp165, cons171, tmp172);
	test_shadowbuf(varposition, samp4, unf175, cons176, tmp168, tmp178);
	shade_only_shadow(tmp172, tmp178, unf181, tmp182);
	shade_only_shadow_diffuse(tmp182, tmp25, tmp130, tmp186);
	lamp_visibility_other(varposition, unf188, tmp189, tmp190, tmp191);
	lamp_falloff_invlinear(cons192, tmp190, tmp194);
	lamp_visibility_spot_circle(unf195, tmp189, tmp197);
	lamp_visibility_spot(cons198, cons199, tmp197, tmp194, tmp202);
	lamp_visibility_clamp(tmp202, tmp204);
	shade_inp(tmp42, tmp189, tmp207);
	shade_visifac(tmp207, tmp204, cons210, tmp211);
	test_shadowbuf(varposition, samp5, unf214, cons215, tmp207, tmp217);
	shade_only_shadow(tmp211, tmp217, unf220, tmp221);
	shade_only_shadow_diffuse(tmp221, tmp25, tmp186, tmp225);
	shade_add(tmp225, tmp147, tmp228);
	mtex_alpha_to_col(tmp228, cons230, tmp231);
	shade_mist_factor(varposition, cons233, cons234, cons235, cons236, tmp237);
	mix_blend(tmp237, tmp231, cons240, tmp241);
	shade_alpha_opaque(tmp241, tmp243);
