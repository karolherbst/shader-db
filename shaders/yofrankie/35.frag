...
varying vec2 var0;
const vec3 cons8 = vec3(2.000000, 2.000000, 1.000000);
const vec3 cons11 = vec3(-0.500000, -0.500000, 0.000000);
uniform sampler2D samp0;
const vec3 cons21 = vec3(0.714613, 0.800000, 0.800000);
const float cons24 = float(1.000000);
const float cons26 = float(0.075000);
const float cons27 = float(1.000000);
const float cons31 = float(0.000000);
const float cons32 = float(1.000000);
const float cons35 = float(0.000000);
const vec3 cons42 = vec3(1.000000, 1.200000, 1.000000);
const vec3 cons45 = vec3(0.000000, -0.100000, 0.000000);
uniform sampler2D samp1;
varying vec3 var1;
const float cons56 = float(0.500000);
varying vec2 var2;
uniform sampler2D samp2;
const float cons73 = float(1.000000);
uniform vec3 unf77;
const float cons88 = float(1.000000);
uniform vec4 unf91;
uniform sampler1D samp3;
const float cons104 = float(1.000000);
const float cons117 = float(0.000000);
const float cons118 = float(52.000000);
uniform vec3 unf122;
const vec3 cons123 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 unf128;
const float cons137 = float(1.000000);
uniform vec4 unf140;
uniform sampler1D samp4;
const float cons153 = float(1.000000);
const float cons166 = float(52.000000);
const float cons168 = float(1.000000);
uniform vec3 unf174;
const vec3 cons175 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 unf181;
const float cons185 = float(20.000000);
uniform vec3 unf188;
const float cons191 = float(0.996195);
const float cons192 = float(0.000571);
const float cons203 = float(1.000000);
uniform sampler2DShadow samp5;
uniform mat4 unf207;
const float cons208 = float(0.005000);
uniform float unf213;
uniform vec3 unf220;
const float cons224 = float(20.000000);
uniform vec3 unf227;
const float cons230 = float(0.996195);
const float cons231 = float(0.000571);
const float cons242 = float(1.000000);
uniform sampler2DShadow samp6;
uniform mat4 unf246;
const float cons247 = float(0.005000);
uniform float unf252;
const float cons262 = float(1.000000);
const float cons265 = float(6.000000);
const float cons266 = float(26.000000);
const float cons267 = float(0.000000);
const float cons268 = float(0.000000);
const vec4 cons272 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	texco_uv(var0, tmp6);
	mtex_mapping_size(tmp6, cons8, tmp9);
	mtex_mapping_ofs(tmp9, cons11, tmp12);
	mtex_image(tmp12, samp0, tmp15, tmp16, tmp17);
	set_rgba(tmp16, tmp19);
	set_value_one(tmp20);
	mtex_rgb_blend(cons21, tmp19.rgb, tmp20, cons24, tmp25);
	math_multiply(cons26, cons27, tmp28);
	mtex_rgbtoint(tmp16, tmp30);
	mtex_value_blend(cons31, cons32, tmp30, tmp28, cons35, tmp36);
	mtex_value_clamp_positive(tmp36, tmp38);
	texco_uv(var0, tmp40);
	mtex_mapping_size(tmp40, cons42, tmp43);
	mtex_mapping_ofs(tmp43, cons45, tmp46);
	mtex_image(tmp46, samp1, tmp49, tmp50, tmp51);
	mtex_nspace_tangent(var1, tmp2, tmp51, tmp55);
	mtex_blend_normal(cons56, tmp2, tmp55, tmp59);
	texco_uv(var2, tmp61);
	mtex_image(tmp61, samp2, tmp64, tmp65, tmp66);
	set_rgba(tmp65, tmp68);
	set_value_one(tmp69);
	mtex_rgb_mul(tmp25, tmp68.rgb, tmp69, cons73, tmp74);
	set_rgb_zero(tmp75);
	set_rgb_zero(tmp76);
	lamp_visibility_sun_hemi(unf77, tmp78, tmp79, tmp80);
	shade_inp(tmp59, tmp78, tmp83);
	shade_is_hemi(tmp83, tmp85);
	shade_visifac(tmp85, tmp80, cons88, tmp89);
	shade_mul_value(tmp89, unf91, tmp92);
	vec_math_dot(tmp4, tmp59, tmp95, tmp96);
	valtorgb(tmp96, samp3, tmp99, tmp100);
	mtex_alpha_from_col(tmp99, tmp102);
	math_multiply(tmp102, cons104, tmp105);
	mix_screen(tmp105, vec4(tmp74, 1.0), tmp99, tmp109);
	shade_madd(vec4(tmp75, 1.0), tmp92, tmp109, tmp113);
	shade_hemi_spec(tmp59, tmp78, tmp4, cons117, cons118, tmp80, tmp120);
	shade_add_spec(tmp120, unf122, cons123, tmp124);
	shade_add(vec4(tmp76, 1.0), vec4(tmp124, 1.0), tmp127);
	lamp_visibility_sun_hemi(unf128, tmp129, tmp130, tmp131);
	shade_inp(tmp59, tmp129, tmp134);
	shade_visifac(tmp134, tmp131, cons137, tmp138);
	shade_mul_value(tmp138, unf140, tmp141);
	vec_math_dot(tmp4, tmp59, tmp144, tmp145);
	valtorgb(tmp145, samp4, tmp148, tmp149);
	mtex_alpha_from_col(tmp148, tmp151);
	math_multiply(tmp151, cons153, tmp154);
	mix_screen(tmp154, vec4(tmp74, 1.0), tmp148, tmp158);
	shade_madd(tmp113, tmp141, tmp158, tmp162);
	shade_cooktorr_spec(tmp59, tmp129, tmp4, cons166, tmp167);
	shade_spec_t(cons168, tmp38, tmp131, tmp167, tmp172);
	shade_add_spec(tmp172, unf174, cons175, tmp176);
	shade_add(tmp127, vec4(tmp176, 1.0), tmp179);
	lamp_visibility_other(varposition, unf181, tmp182, tmp183, tmp184);
	lamp_falloff_invlinear(cons185, tmp183, tmp187);
	lamp_visibility_spot_circle(unf188, tmp182, tmp190);
	lamp_visibility_spot(cons191, cons192, tmp190, tmp187, tmp195);
	lamp_visibility_clamp(tmp195, tmp197);
	shade_inp(tmp59, tmp182, tmp200);
	shade_visifac(tmp200, tmp197, cons203, tmp204);
	test_shadowbuf(varposition, samp5, unf207, cons208, tmp200, tmp210);
	shade_only_shadow(tmp204, tmp210, unf213, tmp214);
	shade_only_shadow_diffuse(tmp214, tmp74, tmp162, tmp218);
	...
	shade_inp(tmp59, tmp221, tmp239);
	shade_visifac(tmp239, tmp236, cons242, tmp243);
	test_shadowbuf(varposition, samp6, unf246, cons247, tmp239, tmp249);
	shade_only_shadow(tmp243, tmp249, unf252, tmp253);
	shade_only_shadow_diffuse(tmp253, tmp74, tmp218, tmp257);
	shade_add(tmp257, tmp179, tmp260);
	mtex_alpha_to_col(tmp260, cons262, tmp263);
	shade_mist_factor(varposition, cons265, cons266, cons267, cons268, tmp269);
	mix_blend(tmp269, tmp263, cons272, tmp273);
	shade_alpha_opaque(tmp273, tmp275);
