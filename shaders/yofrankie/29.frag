...
const float cons47 = float(1.000000);
const float cons49 = float(1.000000);
const float cons50 = float(0.000000);
uniform vec4 unfobcolor;
varying vec3 varposition;
const float cons61 = float(6.000000);
const float cons62 = float(26.000000);
const float cons63 = float(0.000000);
const float cons64 = float(0.000000);
const vec4 cons68 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	texco_orco(var0, tmp2);
	mtex_2d_mapping(tmp2, tmp4);
	mtex_image(tmp4, samp0, tmp7, tmp8, tmp9);
	math_multiply(cons10, cons11, tmp12);
	set_rgba(tmp8, tmp14);
	mtex_rgb_blend(cons15, tmp14.rgb, cons17, tmp12, tmp19);
	mtex_alpha_from_col(tmp8, tmp21);
	mtex_value_blend(cons22, cons23, tmp21, cons25, cons26, tmp27);
	mtex_value_clamp(tmp27, tmp29);
	texco_uv(var1, tmp31);
	mtex_image(tmp31, samp0, tmp34, tmp35, tmp36);
	set_rgba(tmp35, tmp38);
	mtex_rgb_add(tmp19, tmp38.rgb, cons41, cons42, tmp43);
	mtex_alpha_from_col(tmp35, tmp45);
	mtex_value_add(tmp29, cons47, tmp45, cons49, cons50, tmp51);
	mtex_value_clamp(tmp51, tmp53);
	mtex_alpha_to_col(vec4(tmp43, 1.0), tmp53, tmp56);
	shade_obcolor(tmp56, unfobcolor, tmp59);
	shade_mist_factor(varposition, cons61, cons62, cons63, cons64, tmp65);
	mix_blend(tmp65, tmp59, cons68, tmp69);
	shade_alpha_obcolor(tmp69, unfobcolor, tmp72);
