...
const vec3 cons15 = vec3(1.000000, 1.000000, 1.000000);
const float cons18 = float(1.000000);
uniform sampler2D samp1;
varying vec3 var1;
const float cons33 = float(0.100000);
uniform vec3 unf37;
const float cons48 = float(0.699345);
uniform vec4 unf51;
uniform sampler1D samp2;
const float cons64 = float(1.000000);
const float cons77 = float(1.000000);
const float cons78 = float(144.000000);
uniform vec3 unf82;
const vec3 cons83 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 unf88;
const float cons97 = float(0.699345);
uniform vec4 unf100;
uniform sampler1D samp3;
const float cons113 = float(1.000000);
const float cons126 = float(144.000000);
const float cons128 = float(1.000000);
const float cons129 = float(1.000000);
uniform vec3 unf134;
...
const float cons163 = float(0.699345);
uniform sampler2DShadow samp4;
uniform mat4 unf167;
const float cons168 = float(0.005000);
uniform float unf173;
uniform vec3 unf180;
const float cons184 = float(20.000000);
uniform vec3 unf187;
const float cons190 = float(0.996195);
const float cons191 = float(0.000571);
const float cons202 = float(0.699345);
uniform sampler2DShadow samp5;
uniform mat4 unf206;
const float cons207 = float(0.005000);
uniform float unf212;
const float cons222 = float(1.000000);
uniform vec4 unfobcolor;
const float cons228 = float(6.000000);
const float cons229 = float(26.000000);
const float cons230 = float(0.000000);
const float cons231 = float(0.000000);
const vec4 cons235 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	mtex_negate_texnormal(tmp26, tmp28);
	mtex_nspace_tangent(var1, tmp2, tmp28, tmp32);
	shade_mul_value(cons33, vec4(tmp19, 1.0), tmp35);
	set_rgb_zero(tmp36);
	lamp_visibility_sun_hemi(unf37, tmp38, tmp39, tmp40);
	shade_inp(tmp32, tmp38, tmp43);
	shade_is_hemi(tmp43, tmp45);
	shade_visifac(tmp45, tmp40, cons48, tmp49);
	shade_mul_value(tmp49, unf51, tmp52);
	vec_math_dot(tmp4, tmp32, tmp55, tmp56);
	valtorgb(tmp56, samp2, tmp59, tmp60);
	mtex_alpha_from_col(tmp59, tmp62);
	math_multiply(tmp62, cons64, tmp65);
	mix_add(tmp65, vec4(tmp19, 1.0), tmp59, tmp69);
	shade_madd(tmp35, tmp52, tmp69, tmp73);
	shade_hemi_spec(tmp32, tmp38, tmp4, cons77, cons78, tmp40, tmp80);
	shade_add_spec(tmp80, unf82, cons83, tmp84);
	shade_add(vec4(tmp36, 1.0), vec4(tmp84, 1.0), tmp87);
	lamp_visibility_sun_hemi(unf88, tmp89, tmp90, tmp91);
	shade_inp(tmp32, tmp89, tmp94);
	shade_visifac(tmp94, tmp91, cons97, tmp98);
	shade_mul_value(tmp98, unf100, tmp101);
	vec_math_dot(tmp4, tmp32, tmp104, tmp105);
	...
	shade_madd(tmp73, tmp101, tmp118, tmp122);
	shade_phong_spec(tmp32, tmp89, tmp4, cons126, tmp127);
	shade_spec_t(cons128, cons129, tmp91, tmp127, tmp132);
	shade_add_spec(tmp132, unf134, cons135, tmp136);
	shade_add(tmp87, vec4(tmp136, 1.0), tmp139);
	...
	shade_inp(tmp32, tmp142, tmp160);
	shade_visifac(tmp160, tmp157, cons163, tmp164);
	test_shadowbuf(varposition, samp4, unf167, cons168, tmp160, tmp170);
	shade_only_shadow(tmp164, tmp170, unf173, tmp174);
	shade_only_shadow_diffuse(tmp174, tmp19, tmp122, tmp178);
	lamp_visibility_other(varposition, unf180, tmp181, tmp182, tmp183);
	lamp_falloff_invlinear(cons184, tmp182, tmp186);
	lamp_visibility_spot_circle(unf187, tmp181, tmp189);
	lamp_visibility_spot(cons190, cons191, tmp189, tmp186, tmp194);
	lamp_visibility_clamp(tmp194, tmp196);
	shade_inp(tmp32, tmp181, tmp199);
	shade_visifac(tmp199, tmp196, cons202, tmp203);
	test_shadowbuf(varposition, samp5, unf206, cons207, tmp199, tmp209);
	shade_only_shadow(tmp203, tmp209, unf212, tmp213);
	shade_only_shadow_diffuse(tmp213, tmp19, tmp178, tmp217);
	shade_add(tmp217, tmp139, tmp220);
	mtex_alpha_to_col(tmp220, cons222, tmp223);
	shade_obcolor(tmp223, unfobcolor, tmp226);
	shade_mist_factor(varposition, cons228, cons229, cons230, cons231, tmp232);
	mix_blend(tmp232, tmp226, cons235, tmp236);
	shade_alpha_opaque(tmp236, tmp238);
	shade_alpha_obcolor(tmp238, unfobcolor, tmp241);
