...
uniform vec3 unf67;
const float cons71 = float(20.000000);
uniform vec3 unf74;
const float cons77 = float(0.996195);
const float cons78 = float(0.000571);
const float cons89 = float(1.000000);
uniform sampler2DShadow samp1;
uniform mat4 unf93;
const float cons94 = float(0.005000);
uniform float unf99;
uniform vec3 unf106;
const float cons110 = float(20.000000);
uniform vec3 unf113;
const float cons116 = float(0.996195);
const float cons117 = float(0.000571);
const float cons128 = float(1.000000);
uniform sampler2DShadow samp2;
uniform mat4 unf132;
const float cons133 = float(0.005000);
uniform float unf138;
uniform vec4 unfobcolor;
const float cons151 = float(6.000000);
const float cons152 = float(26.000000);
const float cons153 = float(0.000000);
const float cons154 = float(0.000000);
const vec4 cons158 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	set_rgb_zero(tmp27);
	lamp_visibility_sun_hemi(unf28, tmp29, tmp30, tmp31);
	shade_inp(tmp2, tmp29, tmp34);
	shade_is_hemi(tmp34, tmp36);
	shade_visifac(tmp36, tmp31, cons39, tmp40);
	shade_mul_value(tmp40, unf42, tmp43);
	shade_madd(vec4(tmp27, 1.0), tmp43, vec4(tmp16, 1.0), tmp47);
	lamp_visibility_sun_hemi(unf48, tmp49, tmp50, tmp51);
	shade_inp(tmp2, tmp49, tmp54);
	shade_visifac(tmp54, tmp51, cons57, tmp58);
	shade_mul_value(tmp58, unf60, tmp61);
	shade_madd(tmp47, tmp61, vec4(tmp16, 1.0), tmp65);
	lamp_visibility_other(varposition, unf67, tmp68, tmp69, tmp70);
	lamp_falloff_invlinear(cons71, tmp69, tmp73);
	lamp_visibility_spot_circle(unf74, tmp68, tmp76);
	lamp_visibility_spot(cons77, cons78, tmp76, tmp73, tmp81);
	lamp_visibility_clamp(tmp81, tmp83);
	shade_inp(tmp2, tmp68, tmp86);
	shade_visifac(tmp86, tmp83, cons89, tmp90);
	test_shadowbuf(varposition, samp1, unf93, cons94, tmp86, tmp96);
	shade_only_shadow(tmp90, tmp96, unf99, tmp100);
	shade_only_shadow_diffuse(tmp100, tmp16, tmp65, tmp104);
	lamp_visibility_other(varposition, unf106, tmp107, tmp108, tmp109);
	lamp_falloff_invlinear(cons110, tmp108, tmp112);
	lamp_visibility_spot_circle(unf113, tmp107, tmp115);
	lamp_visibility_spot(cons116, cons117, tmp115, tmp112, tmp120);
	lamp_visibility_clamp(tmp120, tmp122);
	shade_inp(tmp2, tmp107, tmp125);
	shade_visifac(tmp125, tmp122, cons128, tmp129);
	test_shadowbuf(varposition, samp2, unf132, cons133, tmp125, tmp135);
	shade_only_shadow(tmp129, tmp135, unf138, tmp139);
	shade_only_shadow_diffuse(tmp139, tmp16, tmp104, tmp143);
	mtex_alpha_to_col(tmp143, tmp26, tmp146);
	shade_obcolor(tmp146, unfobcolor, tmp149);
	shade_mist_factor(varposition, cons151, cons152, cons153, cons154, tmp155);
	mix_blend(tmp155, tmp149, cons158, tmp159);
	shade_alpha_obcolor(tmp159, unfobcolor, tmp162);
