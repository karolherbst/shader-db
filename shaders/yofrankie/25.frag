...
const vec3 cons15 = vec3(0.552941, 0.180392, 0.050980);
const float cons18 = float(1.000000);
uniform sampler2D samp1;
varying vec3 var1;
uniform vec3 unf33;
const float cons46 = float(0.870586);
uniform vec4 unf49;
uniform sampler1D samp2;
const float cons62 = float(1.000000);
const float cons75 = float(1.756183);
const float cons76 = float(150.000000);
uniform vec3 unf80;
const vec3 cons81 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 unf86;
const float cons97 = float(0.870586);
uniform vec4 unf100;
uniform sampler1D samp3;
const float cons113 = float(1.000000);
const float cons126 = float(150.000000);
const float cons128 = float(1.000000);
const float cons129 = float(1.756183);
uniform vec3 unf134;
const vec3 cons135 = vec3(1.000000, 1.000000, 1.000000);
uniform vec3 unf141;
const float cons145 = float(20.000000);
uniform vec3 unf148;
const float cons151 = float(0.996195);
const float cons152 = float(0.000571);
const float cons165 = float(0.870586);
uniform sampler2DShadow samp4;
uniform mat4 unf169;
const float cons170 = float(0.005000);
uniform float unf175;
uniform vec3 unf182;
const float cons186 = float(20.000000);
uniform vec3 unf189;
const float cons192 = float(0.996195);
const float cons193 = float(0.000571);
const float cons206 = float(0.870586);
uniform sampler2DShadow samp5;
uniform mat4 unf210;
const float cons211 = float(0.005000);
uniform float unf216;
const float cons226 = float(1.000000);
uniform vec4 unfobcolor;
const float cons232 = float(6.000000);
const float cons233 = float(26.000000);
const float cons234 = float(0.000000);
const float cons235 = float(0.000000);
const vec4 cons239 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	mtex_nspace_tangent(var1, tmp2, tmp26, tmp30);
	set_rgb_zero(tmp31);
	set_rgb_zero(tmp32);
	lamp_visibility_sun_hemi(unf33, tmp34, tmp35, tmp36);
	shade_inp(tmp30, tmp34, tmp39);
	shade_is_hemi(tmp39, tmp41);
	shade_cubic(tmp41, tmp43);
	shade_visifac(tmp43, tmp36, cons46, tmp47);
	shade_mul_value(tmp47, unf49, tmp50);
	vec_math_dot(tmp4, tmp30, tmp53, tmp54);
	valtorgb(tmp54, samp2, tmp57, tmp58);
	mtex_alpha_from_col(tmp57, tmp60);
	math_multiply(tmp60, cons62, tmp63);
	mix_add(tmp63, vec4(tmp19, 1.0), tmp57, tmp67);
	shade_madd(vec4(tmp31, 1.0), tmp50, tmp67, tmp71);
	shade_hemi_spec(tmp30, tmp34, tmp4, cons75, cons76, tmp36, tmp78);
	shade_add_spec(tmp78, unf80, cons81, tmp82);
	shade_add(vec4(tmp32, 1.0), vec4(tmp82, 1.0), tmp85);
	lamp_visibility_sun_hemi(unf86, tmp87, tmp88, tmp89);
	shade_inp(tmp30, tmp87, tmp92);
	shade_cubic(tmp92, tmp94);
	shade_visifac(tmp94, tmp89, cons97, tmp98);
	shade_mul_value(tmp98, unf100, tmp101);
	vec_math_dot(tmp4, tmp30, tmp104, tmp105);
	valtorgb(tmp105, samp3, tmp108, tmp109);
	mtex_alpha_from_col(tmp108, tmp111);
	math_multiply(tmp111, cons113, tmp114);
	mix_add(tmp114, vec4(tmp19, 1.0), tmp108, tmp118);
	shade_madd(tmp71, tmp101, tmp118, tmp122);
	shade_phong_spec(tmp30, tmp87, tmp4, cons126, tmp127);
	shade_spec_t(cons128, cons129, tmp89, tmp127, tmp132);
	shade_add_spec(tmp132, unf134, cons135, tmp136);
	shade_add(tmp85, vec4(tmp136, 1.0), tmp139);
	lamp_visibility_other(varposition, unf141, tmp142, tmp143, tmp144);
	lamp_falloff_invlinear(cons145, tmp143, tmp147);
	lamp_visibility_spot_circle(unf148, tmp142, tmp150);
	lamp_visibility_spot(cons151, cons152, tmp150, tmp147, tmp155);
	lamp_visibility_clamp(tmp155, tmp157);
	shade_inp(tmp30, tmp142, tmp160);
	shade_cubic(tmp160, tmp162);
	shade_visifac(tmp162, tmp157, cons165, tmp166);
	test_shadowbuf(varposition, samp4, unf169, cons170, tmp160, tmp172);
	shade_only_shadow(tmp166, tmp172, unf175, tmp176);
	shade_only_shadow_diffuse(tmp176, tmp19, tmp122, tmp180);
	lamp_visibility_other(varposition, unf182, tmp183, tmp184, tmp185);
	lamp_falloff_invlinear(cons186, tmp184, tmp188);
	lamp_visibility_spot_circle(unf189, tmp183, tmp191);
	lamp_visibility_spot(cons192, cons193, tmp191, tmp188, tmp196);
	lamp_visibility_clamp(tmp196, tmp198);
	shade_inp(tmp30, tmp183, tmp201);
	shade_cubic(tmp201, tmp203);
	shade_visifac(tmp203, tmp198, cons206, tmp207);
	test_shadowbuf(varposition, samp5, unf210, cons211, tmp201, tmp213);
	shade_only_shadow(tmp207, tmp213, unf216, tmp217);
	shade_only_shadow_diffuse(tmp217, tmp19, tmp180, tmp221);
	shade_add(tmp221, tmp139, tmp224);
	mtex_alpha_to_col(tmp224, cons226, tmp227);
	shade_obcolor(tmp227, unfobcolor, tmp230);
	shade_mist_factor(varposition, cons232, cons233, cons234, cons235, tmp236);
	mix_blend(tmp236, tmp230, cons239, tmp240);
	shade_alpha_opaque(tmp240, tmp242);
	shade_alpha_obcolor(tmp242, unfobcolor, tmp245);
