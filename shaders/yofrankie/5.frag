...
const float cons29 = float(6.000000);
const float cons30 = float(26.000000);
const float cons31 = float(0.000000);
const float cons32 = float(0.000000);
const vec4 cons36 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	texco_uv(var0, tmp2);
	mtex_image(tmp2, samp0, tmp5, tmp6, tmp7);
	mtex_rgb_stencil(cons8, tmp6, tmp10, tmp11);
	mtex_alpha_from_col(tmp11, tmp13);
	mtex_value_blend(cons14, cons15, tmp13, tmp10, cons18, tmp19);
	mtex_value_clamp(tmp19, tmp21);
	mtex_alpha_to_col(cons22, tmp21, tmp24);
	shade_obcolor(tmp24, unfobcolor, tmp27);
	shade_mist_factor(varposition, cons29, cons30, cons31, cons32, tmp33);
	mix_blend(tmp33, tmp27, cons36, tmp37);
	shade_alpha_obcolor(tmp37, unfobcolor, tmp40);
