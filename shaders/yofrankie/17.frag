...
const float cons61 = float(0.801932);
uniform vec3 unf71;
const float cons80 = float(1.000000);
uniform vec4 unf83;
uniform sampler1D samp3;
const float cons96 = float(0.801932);
uniform vec3 unf107;
const float cons111 = float(20.000000);
uniform vec3 unf114;
const float cons117 = float(0.996195);
const float cons118 = float(0.000571);
const float cons129 = float(1.000000);
uniform sampler2DShadow samp4;
uniform mat4 unf133;
const float cons134 = float(0.005000);
uniform float unf139;
uniform vec3 unf146;
const float cons150 = float(20.000000);
uniform vec3 unf153;
const float cons156 = float(0.996195);
const float cons157 = float(0.000571);
const float cons168 = float(1.000000);
uniform sampler2DShadow samp5;
uniform mat4 unf172;
const float cons173 = float(0.005000);
uniform float unf178;
const float cons185 = float(1.000000);
const float cons188 = float(6.000000);
const float cons189 = float(26.000000);
const float cons190 = float(0.000000);
const float cons191 = float(0.000000);
const vec4 cons195 = vec4(0.906274, 0.882745, 0.992157, 0.000000);
...
void main(void)
{
	...
	vcol_attribute(var0, tmp6);
	texco_uv(var1, tmp8);
	mtex_image(tmp8, samp0, tmp11, tmp12, tmp13);
	set_rgba(tmp12, tmp15);
	set_value_one(tmp16);
	mtex_rgb_mul(tmp6.rgb, tmp15.rgb, tmp16, cons20, tmp21);
	texco_uv(var1, tmp23);
	mtex_image(tmp23, samp1, tmp26, tmp27, tmp28);
	mtex_nspace_tangent(var2, tmp2, tmp28, tmp32);
	set_rgb_zero(tmp33);
	lamp_visibility_sun_hemi(unf34, tmp35, tmp36, tmp37);
	shade_inp(tmp32, tmp35, tmp40);
	shade_is_hemi(tmp40, tmp42);
	shade_visifac(tmp42, tmp37, cons45, tmp46);
	shade_mul_value(tmp46, unf48, tmp49);
	vec_math_dot(tmp4, tmp32, tmp52, tmp53);
	valtorgb(tmp53, samp2, tmp56, tmp57);
	mtex_alpha_from_col(tmp56, tmp59);
	math_multiply(tmp59, cons61, tmp62);
	mix_overlay(tmp62, vec4(tmp21, 1.0), tmp56, tmp66);
	shade_madd(vec4(tmp33, 1.0), tmp49, tmp66, tmp70);
	lamp_visibility_sun_hemi(unf71, tmp72, tmp73, tmp74);
	shade_inp(tmp32, tmp72, tmp77);
	shade_visifac(tmp77, tmp74, cons80, tmp81);
	shade_mul_value(tmp81, unf83, tmp84);
	vec_math_dot(tmp4, tmp32, tmp87, tmp88);
	valtorgb(tmp88, samp3, tmp91, tmp92);
	mtex_alpha_from_col(tmp91, tmp94);
	math_multiply(tmp94, cons96, tmp97);
	mix_overlay(tmp97, vec4(tmp21, 1.0), tmp91, tmp101);
	shade_madd(tmp70, tmp84, tmp101, tmp105);
	lamp_visibility_other(varposition, unf107, tmp108, tmp109, tmp110);
	lamp_falloff_invlinear(cons111, tmp109, tmp113);
	lamp_visibility_spot_circle(unf114, tmp108, tmp116);
	lamp_visibility_spot(cons117, cons118, tmp116, tmp113, tmp121);
	lamp_visibility_clamp(tmp121, tmp123);
	shade_inp(tmp32, tmp108, tmp126);
	shade_visifac(tmp126, tmp123, cons129, tmp130);
	test_shadowbuf(varposition, samp4, unf133, cons134, tmp126, tmp136);
	shade_only_shadow(tmp130, tmp136, unf139, tmp140);
	shade_only_shadow_diffuse(tmp140, tmp21, tmp105, tmp144);
	lamp_visibility_other(varposition, unf146, tmp147, tmp148, tmp149);
	lamp_falloff_invlinear(cons150, tmp148, tmp152);
	lamp_visibility_spot_circle(unf153, tmp147, tmp155);
	lamp_visibility_spot(cons156, cons157, tmp155, tmp152, tmp160);
	lamp_visibility_clamp(tmp160, tmp162);
	shade_inp(tmp32, tmp147, tmp165);
	shade_visifac(tmp165, tmp162, cons168, tmp169);
	test_shadowbuf(varposition, samp5, unf172, cons173, tmp165, tmp175);
	shade_only_shadow(tmp169, tmp175, unf178, tmp179);
	shade_only_shadow_diffuse(tmp179, tmp21, tmp144, tmp183);
	mtex_alpha_to_col(tmp183, cons185, tmp186);
	shade_mist_factor(varposition, cons188, cons189, cons190, cons191, tmp192);
	mix_blend(tmp192, tmp186, cons195, tmp196);
	shade_alpha_opaque(tmp196, tmp198);
